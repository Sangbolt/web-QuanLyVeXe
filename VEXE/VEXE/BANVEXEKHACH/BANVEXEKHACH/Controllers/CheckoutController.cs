...
namespace VEXE.Controllers
{
    public class CheckoutController : Controller
    {
        ...
        [HttpPost]
        public ActionResult ThanhToanOnline(string name, string email, string phone, int total, string payment)
        {
            System.Diagnostics.Debug.WriteLine($"Payment Method: {payment}");
            if (payment == "credit")
            {
                string qrCodeUrl = $"https://api.vietqr.io/image/970436-1027039052-UffU2Qe.jpg?accountName=TRAN%20XUAN%20VIET%20TRUONG&amount={total}&addInfo=VE%20XE%20PHUONG%20TRANG";
                ViewBag.QrCodeUrl = qrCodeUrl;
                return View("QrCodeView");
            }
            else if (payment == "checkinPayment")
            {
        ...
        public ActionResult checkOutComfin()
        {
            ViewBag.Message = "Thanh toán của bạn đã được xác nhận.";
            return View();
        }
        ...
        {
            ViewBag.Message = "Đặt vé thành công. Cảm ơn bạn đã thanh toán!";
            return View();
        }
    }
}